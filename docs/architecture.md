...
`debts`, `debt_preferences`, `debt_check_ins`, and `debt_check_in_balances` are owner- and entity-scoped, RLS-protected records. Money is stored in signed integer cents and annual interest in integer basis points. Debt records are liabilities, not bank-account balances or planned payments; onboarding or editing a debt does not create a transaction, alter safe-to-spend, or contact a credit provider. Paid and archived debts are soft-closed so milestone history remains intact.
...
The simulator applies monthly interest, pays every entered minimum, then directs the remaining fixed monthly capacity according to the chosen order. Freed minimums therefore roll into subsequent debts. It returns the month-by-month remaining balance, cumulative interest, active target, debt payoff events, and 25/50/75/100% milestones. Avalanche, snowball, and hybrid are evaluated against identical monthly capacity for a comparable time-and-interest view. Existing planned debt-payment lines are compared with the estimate and can produce a warning, but only entered contractual minimums determine basic affordability. The simulator stops at 600 months and treats failure to amortize, unaffordable minimums, or arrears as a professional-support case. For a personal entity this route links to the NCR register for debt counselling; for a business entity the UI instead calls for qualified restructuring advice. A consolidation offer is never inferred: users must enter its APR, term, and fees, and it is recommended only when the calculated payment is affordable and estimated total cost is at least 5% lower (and at least R100 lower) than avalanche. These projections are educational estimates; lender statements and formal advice remain authoritative.

`record_debt_check_in` is a security-invoker RPC that locks the entity's active debts, requires each one exactly once, replaces a same-month snapshot atomically, and updates the current debt balances. A zero balance closes a debt as paid off; correcting the same month's snapshot can reopen it. The Android chart uses up to twelve check-ins as its solid actual series and the recalculated projection as its dashed forecast. Reminder preferences are stored with the entity's debt preferences; WorkManager schedules generic local reminders and the app retains a separate in-app due state.

`debt_spending_history` is a security-invoker aggregation over posted outgoing transactions. It excludes transfers and reversals and exposes categorized and uncategorized monthly totals without bypassing transaction RLS. Android considers only categories with a current flexible limit, excludes savings/debt/transfer system categories, and computes a median across the three completed months before the current month. Suggestions require at least two months and 75% categorized coverage. A scenario supplies additional capacity to an in-memory projection only; budget mutations still go through the existing explicit budget editor.
...
- The collector does not publish transaction notifications. Android requests notification permission only after a user enables optional generic debt check-in reminders.
